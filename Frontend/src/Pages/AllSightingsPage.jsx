...
function AllSightingsPage() {
      ...
      .then((updatedSighting) => {
        setSightings((prevSightings) =>
          prevSightings.map((sighting) =>
            sighting.id === id
              ? { ...sighting, description: updatedSighting.description }
              : sighting
          )
        );
        setEditingId(null);
  ...
  const handleDelete = (id) => {
    fetch(`http://localhost:8080/api/sightings/delete/${id}`, {
      method: "DELETE",
    })
      .then((response) => {
        if (!response.ok) {
            ...
            <div key={sighting.id} className="sighting-card">
              {sighting.imageUrl && (
                <img
                  ...
                  alt={`Sighting in ${sighting.location}`}
              ...
              {editingId === sighting.id ? (
                <>
                  ...
                  />
                  <Button label="Cancel" onClick={() => setEditingId(null)} />
                </>
              ) : (
                <>
                  <p>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleEditClick(sighting.id, sighting.description)
                    }
                  />
                </>
              )}
